...
@WebMvcTest(AccountController.class)//특정 컨트롤로만 격리해 테스트해 봄. :)
class AccountControllerTest {
    ...
    void successGetAccountsByUserId() throws Exception {
        ...
        mockMvc.perform(get("/account?user_id=1"))
                ...
                .andExpect(jsonPath("$[1].accountNumber").value("2345678910"))
                .andExpect(jsonPath("$[1].balance").value(2000L))
                .andExpect(jsonPath("$[2].accountNumber").value("3456789120"))
    ...
    void successGetAccount() throws Exception {
        ...
        mockMvc.perform(get("/account/876"))
                .andDo(print())//응답값을 보여줌
                .andExpect(jsonPath("$.accountNumber").value("3456"))
                .andExpect(jsonPath("$.accountStatus").value("IN_USE"))
                .andExpect(status().isOk());//외우는 과정이 필요

    }

    @Test
    @DisplayName("계좌 불러오기 실패")
